...
re_int = re.compile(
    r'([+-]?[0-9]*[.]?[0-9]+)([@][0-9]*[.][0-9]+)?[~]([+-]?[0-9]*[.]?[0-9]+)([@][0-9]*[.][0-9]+)?', flags=re.MULTILINE)
...
def hijack_init(self, all_prompts, all_seeds, all_subseeds):
    ...
    def toLatent(img):
        ...
        res = self.sd_model.get_first_stage_encoding(
            self.sd_model.encode_first_stage(res))
    ...
    if self.image_mask and self.inpaint_full_res:
        ...
        latent2 = toLatent(images.resize_image(
            2, self.init_img2.crop(crop_region), self.width, self.height))
    else:
        latent2 = toLatent(self.init_img2)
    self.init_latent = self.init_latent * \
        (1.-self.interpolate_ratio) + latent2*self.interpolate_ratio
    del latent2
    if self.mixin_img:
        ...
        for i in self.mixin_img:
            if self.image_mask and self.inpaint_full_res:
                latent_i = toLatent(images.resize_image(
                    2, i.crop(crop_region), self.width, self.height))
            else:
                latent_i = toLatent(i)
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        init_img2 = gr.Image(label="alternate img2img imgage", elem_id="img2img_image_alternate",
                             show_label=False, source="upload", interactive=True, type="pil")
        ...
        with gr.Box(visible=False) as loopback_box:
            ...
            loopback_loops = gr.Slider(
                minimum=1, maximum=32, step=1, label='Refinement loops', value=1)
            loopback_alpha = gr.Slider(
                minimum=0.0, maximum=1.0, step=0.005, label='Loopback alpha', value=0.2)
            border_alpha = gr.Slider(
                minimum=0.0, maximum=1.0, step=0.005, label='Border alpha', value=0.1)
            blend_strides = gr.Slider(
                minimum=0, maximum=32, step=1, label='Blending strides', value=1)
        ...
        with gr.Row() as settings_row:
            ...
            interpolate_varseed = gr.Checkbox(
                label='Interpolate VarSeed', value=False)
            paste_on_mask = gr.Checkbox(label='Paste on mask', value=False)
            inpaint_all = gr.Checkbox(label='Inpaint all', value=False)
            interpolate_latent = gr.Checkbox(
                label='Interpolate in latent', value=False)

        loopback_toggle.change(change_visibility, show_progress=False, inputs=[
                               loopback_toggle], outputs=loopback_vis)
    ...
    def run(self, p, init_img2, i_values, loopback_alpha, border_alpha, loopback_loops, blend_strides, loopback_toggle, reuse_seed, one_grid, interpolate_varseed, paste_on_mask, inpaint_all, interpolate_latent):
        ...
        if p.image_mask is None:
            init_img = images.resize_image(
                p.resize_mode, p.init_images[0], p.width, p.height)
            if init_img2:
                init_img2 = images.resize_image(
                    p.resize_mode, init_img2, p.width, p.height)
        else:
            ...
            if p.mask_blur > 0:
                init_mask = init_mask.filter(
                    ImageFilter.GaussianBlur(p.mask_blur))
            ...
            if p.inpaint_full_res:
                ...
                if init_img2:
                    if paste_on_mask:
                        ...
                        x1, y1, x2, y2 = masking.get_crop_region(
                            np.array(init_mask), 0)
                        init_img_copy.paste(images.resize_image(
                            p.resize_mode, init_img2, x2-x1, y2-y1), (x1, y1))
                        init_img2 = init_img_copy
                    else:
                        w, h = init_img.size
                        init_img2 = images.resize_image(
                            p.resize_mode, init_img2, w, h)
            else:
                init_img = images.resize_image(
                    p.resize_mode, p.init_images[0], p.width, p.height)
                init_mask = images.resize_image(
                    p.resize_mode, init_mask, p.width, p.height)
                if init_img2:
                    if paste_on_mask:
                        init_img_copy = init_img.copy()
                        x1, y1, x2, y2 = masking.get_crop_region(
        ...
        state.job_count = len(x) * batch_count * \
            (loopback_loops + 1 if loopback_toggle else 1)
        ...
        def process_list(img_in):
            ...
            for i in range(len(x)):
                ...
                if interpolate_latent:
                    ...
                    # mix with previous level and neighbors
                    if img_in and i != 0 and i != len(x)-1:
                        pc.mixin_ratio = loopback_alpha
                        pc.mixin_img = [img_in[j] for j in set(range(len(img_in))) & set(
                            range(i-blend_strides, i+blend_strides+1))]
                    elif img_in:
                ...
                if init_mask and not inpaint_all:  # test
                    res.append(Image.composite(
                        processed.images[0], init_img, init_mask))
                else:
        ...
        def blend_images(base, pre, alpha, border_alpha, strides):
            ...
            for i in range(len(base)):
                if i == 0 or i == len(base)-1 or strides == 0:
                    res.append(blend_images_fix([base[i], pre[i]], [
                               1-border_alpha, border_alpha]))
                else:
                    l = min(min(i, strides), min(len(base)-i-1, strides))
                    indices = list(set(range(i-l, i+l+1))
                                   & set(range(len(base))))
                    imgs = [base[i]]
                    weights = [1-alpha]
        ...
        for n in range(batch_count):
            if interpolate_latent:
                ...
                level0 = [Image.composite(blend_images_fix([init_img, init_img2], [
                                          1-min(1, max(0, i)), min(1, max(0, i))]), init_img, init_mask) for i in x]
            else:
                # level0 = [Image.blend(init_img, init_img2, min(1,max(0,i))) for i in x]
                level0 = [blend_images_fix([init_img, init_img2], [
                                           1-min(1, max(0, i)), min(1, max(0, i))]) for i in x]
                    ...
                    if interpolate_latent:
                        if init_mask:
                            if crop_region is None:
                                cur_level_resized = [images.resize_image(
                                    p.resize_mode, j, p.width, p.height) for j in cur_level]
                            else:
                                cur_level_resized = [
                                    j.crop(crop_region) for j in cur_level]
                                cur_level_resized = [images.resize_image(
                                    p.resize_mode, j, p.width, p.height) for j in cur_level_resized]
                        else:
                            cur_level_resized = cur_level
                        cur_level = process_list(cur_level_resized)
                    else:
                        cur_level = process_list(blend_images(
                            level0, cur_level, loopback_alpha, border_alpha, blend_strides))
        ...
        if one_grid:
            grid = images.image_grid(
                all_images_grid, rows=batch_count*(loopback_loops+1 if loopback_toggle else 1))
            if opts.grid_save:
                images.save_image(grid, p.outpath_grids, "grid", initial_seed, p.prompt, opts.grid_format,
                                  info=info, short_filename=not opts.grid_extended_filename, grid=True, p=p)
        ...
        processed = Processed(p, all_images, seed=initial_seed, info=initial_info,
                              all_seeds=all_seeds, all_prompts=all_prompts, infotexts=all_infos)
